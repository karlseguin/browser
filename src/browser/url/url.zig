...
pub const URL = struct {
    ...
    pub fn constructor(state: *SessionState, url: []const u8, base: ?[]const u8) !URL {
        const raw = try std.mem.concat(state.arena, u8, &[_][]const u8{ url, base orelse "" });
    ...
    pub fn deinit(self: *URL, state: *SessionState) void {
        self.search_params.deinit();
        state.arena.free(self.rawuri);
    }
...
pub const URLSearchParams = struct {
    ...
    pub fn constructor(state: *SessionState, init: ?[]const u8) !URLSearchParams {
        return .{
            .values = try query.parseQuery(state.arena, init orelse ""),
        };
    }
